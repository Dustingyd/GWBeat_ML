...
class DatasetGenerator(Dataset):
    def __init__(self, fs=8192, T=10, snr=20,
                 detectors=['H1', 'L1'],
        ...
        self.generate(nsample_perepoch, Nnoise)  # pre-generate sampels

    def generate(self, Nblock, Nnoise=25):
        # 生成数据的函数
        ...
        ts, par = sim_data(self.fs, self.T, self.snr, self.detectors, Nnoise, size=Nblock,verbose=False)
...
class MyNet(nn.Module):
    def __init__(self):
        ...
        num = 10
        Nfilters = [8, 16, 16, 32, 64, 64, 128, 128, 256, 256]
        filter_size = [(1, 32)] + [(1, 16)] * 3 + [(1, 8)] * 2 + [(1, 4)] * 2 + [(1, 2)] * 2
        filter_stride = [(1, 1)] * num
        dilation = [(1, 1)] * num
        pooling = [1, 0, 0, 0, 1, 0, 0, 1, 1, 1]
        pool_size = [[1, 8]] + [(1, 1)] * 3 + [[1, 6]] + [(1, 1)] * 2 + [[1, 4]] * 3
        pool_stride = [[1, 8]] + [(1, 1)] * 3 + [[1, 6]] + [(1, 1)] * 2 + [[1, 4]] * 3
        ...
        # 添加全连接层，输入维度为self.flatten_dim，输出维度为64 
        self.layers.append(nn.Linear(26624, 64))
        # 添加ELU激活函数，alpha参数为0.01
        self.layers.append(nn.ELU(0.01))
        # 添加Dropout层，丢弃率为0.5
...
def train(net, lr, nsample_perepoch, epoch, total_epochs,
    ...
    """训练函数"""
    # 设置优化器参数
    loss_func = nn.CrossEntropyLoss() #nn.CrossEntropyLoss()  # 定义损失函数
