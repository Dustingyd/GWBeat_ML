...
def gen_beat(t, M0, z, theta, iniphi, alpha0, as0, Ms0RM0_Dom, Ms0RM0_Sub):
    ...
    res = GWs_SR(M0, Ms2p, Ms3p, z, alpha0, theta, iniphi, t/t_unit)
    return res
...
def gen_par(fs,T_obs,verbose=True):
    """
    ...
    if verbose:
        print('{}: selected bh-condensate cos(inclination) = {}'.format(time.asctime(),np.cos(iota)))

    # generate polarisation angle
    psi = 2.0*np.pi*np.random.rand()
    if verbose:
        print('{}: selected bh-condensate polarisation = {}'.format(time.asctime(),psi))

    # generate reference phase
    phi = 2.0*np.pi*np.random.rand()
    if verbose:
        print('{}: selected bh-condensate reference phase = {}'.format(time.asctime(),phi))

    # pick sky position - uniform on the 2-sphere
    ra = 2.0*np.pi*np.random.rand()
    dec = np.arcsin(-1.0 + 2.0*np.random.rand())
    if verbose:
        print('{}: selected bh-condensate sky position = {},{}'.format(time.asctime(),ra,dec))

    # generate inital condensate mass
    Ms0_0 = 10**(-3 - 5*np.random.rand())
    Ms0_1 = 10**(-3 - 5*np.random.rand())
    if verbose:
        print('{}: selected inital condensate mass = {},{}'.format(time.asctime(),Ms0_0,Ms0_1))

    # generate redshift
    z = 10**(-3 + 4*np.random.rand())
    if verbose:
        print('{}: selected redshift = {}'.format(time.asctime(),z))

    # generate inital bh mass
    M0_min = 5.0
    M0_max = 0.0
    
    while M0_max < M0_min:
        # generate inital BH spin
        as0 = 0.998 #* np.random.rand()
        # generate GW frequency
        f = 10**(1.1 + 2*np.random.rand())
        # define distribution params
        M0_max = M_times_f(1, as0) / f
        M0_min = 10 * np.sqrt(0.1 / (f/10**3)**3 / 0.831)
        if M0_min < 5.0:
            M0_min = 5.0
        #print(f,M0_min,M0_max)

                        
    M0 = np.exp(np.log(M0_min) + np.log(M0_max/M0_min)*np.random.rand())

    alpha0 = 0.0000154744 * M0 * f

    if verbose:
        print('{}: selected bh spin = {}'.format(time.asctime(),as0))
    if verbose:
        print('{}: selected GW frequency / Hz = {}'.format(time.asctime(),f))
    if verbose:
        print('{}: selected bh mass (M_sun) = {}'.format(time.asctime(),M0))
    if verbose:
        print('{}: selected mass coupling = {}'.format(time.asctime(),alpha0))
...
def gen_signal(fs,T_obs,psds,pars,snr=1.0,dets=['H1'],verbose=True):
    """
...
def sim_data(fs,T_obs,snr=1.0,dets=['H1'],Nnoise=25,size=1000,verbose=True):
    """
    ...
    while cnt < size:
        ...
        # generate a single new timeseries
        par_new = gen_par(fs,T_obs,verbose=verbose)
        ts_new,_,_ = gen_signal(fs,T_obs,psds,par_new,snr=snr,dets=dets,verbose=verbose)
